...
$(document).ready(function() {
  ...
  function insertWarehouse() {
    $("#add_warehouse").on(`click`, `button[id='add_btn']`, function(e) {
      ...
      $.ajax({
        type: `POST`,
        url: DOMAIN_API + DOMAIN_WAREHOUSE + REQUEST_SAVE,
        contentType: `application/json`,
        dataType: `json`,
        data: JSON.stringify({
          name: name
        ...
        success: function(res) {
          ...
          elAdd
            .find(`#danger_warehouse`)
            .removeClass(`d-block`)
            .addClass(`d-none`)
            .html(``);

          elAdd
            .find(`#success_warehouse`)
            .removeClass(`d-none`)
            .addClass(`d-block`)
            .html(suc);
  ...
  function blockEditWarehouse() {
    $(`#warehouses`).delegate(
      `button[data-target='#edit_warehouse']`,
      `click`,
      function(e) {
  ...
  function blockDeleteWarehouse() {
    $(`#warehouses`).delegate(
      `button[data-target='#delete_warehouse']`,
      `click`,
      function(e) {
        let id = $(this)
          .closest(`tbody tr`)
  ...
  function deleteWarehouse() {
    $("#delete_warehouse").on(`click`, `button[id='delete_btn']`, function(e) {
      ...
      $.ajax({
        type: `POST`,
        url: DOMAIN_API + DOMAIN_WAREHOUSE + REQUEST_DELETE,
        contentType: `application/json`,
        dataType: `json`,
        data: JSON.stringify({
          id: id
