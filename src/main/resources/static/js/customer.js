...
$(document).ready(function() {
  ...
  function getCustomer() {
    $.ajax({
      type: `POST`,
      url: DOMAIN_API + DOMAIN_CUSTOMER + REQUEST_FILTER,
  ...
  function insertCustomer() {
    $("#add_customer").on(`click`, `button[id='add_btn']`, function(e) {
      ...
      $.ajax({
        type: `POST`,
        url: DOMAIN_API + DOMAIN_CUSTOMER + REQUEST_SAVE,
        contentType: `application/json`,
        dataType: `json`,
        data: JSON.stringify({
          name: name,
        ...
        success: function(res) {
          ...
          elAdd
            .find(`#danger_customer`)
            .removeClass(`d-block`)
            .addClass(`d-none`)
            .html(``);

          elAdd
            .find(`#success_customer`)
            .removeClass(`d-none`)
            .addClass(`d-block`)
            .html(suc);
  ...
  function blockEditCustomer() {
    $(`#customers`).delegate(
      `button[data-target='#edit_customer']`,
      `click`,
      function(e) {
  ...
  function blockDeleteCustomer() {
    $(`#customers`).delegate(
      `button[data-target='#delete_customer']`,
      `click`,
      function(e) {
        let id = $(this)
          .closest(`tbody tr`)
  ...
  function deleteCustomer() {
    $("#delete_customer").on(`click`, `button[id='delete_btn']`, function(e) {
      ...
      $.ajax({
        type: `POST`,
        url: DOMAIN_API + DOMAIN_CUSTOMER + REQUEST_DELETE,
        contentType: `application/json`,
        dataType: `json`,
        data: JSON.stringify({
          id: id
