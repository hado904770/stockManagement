...
$(document).ready(function() {
  ...
  function getProvider() {
    $.ajax({
      type: `POST`,
      url: DOMAIN_API + DOMAIN_PROVIDER + REQUEST_FILTER,
  ...
  function insertProvider() {
    $("#add_provider").on(`click`, `button[id='add_btn']`, function(e) {
      ...
      $.ajax({
        type: `POST`,
        url: DOMAIN_API + DOMAIN_PROVIDER + REQUEST_SAVE,
        contentType: `application/json`,
        dataType: `json`,
        data: JSON.stringify({
          name: name,
        ...
        success: function(res) {
          ...
          elAdd
            .find(`#danger_provider`)
            .removeClass(`d-block`)
            .addClass(`d-none`)
            .html(``);

          elAdd
            .find(`#success_provider`)
            .removeClass(`d-none`)
            .addClass(`d-block`)
            .html(suc);
  ...
  function blockEditProvider() {
    $(`#providers`).delegate(
      `button[data-target='#edit_provider']`,
      `click`,
      function(e) {
  ...
  function blockDeleteProvider() {
    $(`#providers`).delegate(
      `button[data-target='#delete_provider']`,
      `click`,
      function(e) {
        let id = $(this)
          .closest(`tbody tr`)
  ...
  function deleteProvider() {
    $("#delete_provider").on(`click`, `button[id='delete_btn']`, function(e) {
      ...
      $.ajax({
        type: `POST`,
        url: DOMAIN_API + DOMAIN_PROVIDER + REQUEST_DELETE,
        contentType: `application/json`,
        dataType: `json`,
        data: JSON.stringify({
          id: id
